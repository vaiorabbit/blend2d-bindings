...
import blend2d_parser

PREFIX = """# Ruby-Blend2D : Yet another Blend2D wrapper for Ruby
#
...
def sanitize_macro(ctx):
    ...
    # refer mapping
    for macro_name, macro_value in ctx.decl_macros.items():
        define_mapping = blend2d_parser.get_define_mapping(macro_name)
        if define_mapping:
    ...
    for struct_name, struct_info in ctx.decl_structs.items():
        ...
        for field in struct_info.fields:
            if str(field.type_kind) == "TypeKind.FUNCTIONNOPROTO": # for BL_DEFINE_OBJECT_DCAST(BLGlyphBuffer) (glyphbuffer.h)
                ...
                field.type_kind = blend2d_parser.get_cindex_ctypes_mapping(str(field.type_kind), field.type_api_name)
    ...
    for typedef_name, typedef_info in ctx.decl_typedefs.items():
        ...
        if typedef_info.func_proto != None:
            for arg in typedef_info.func_proto.args:
                arg.type_kind = blend2d_parser.get_cindex_ctypes_mapping(str(arg.type_kind), arg.type_name)
            typedef_info.func_proto.retval.type_kind = blend2d_parser.get_cindex_ctypes_mapping(str(typedef_info.func_proto.retval.type_kind), typedef_info.func_proto.retval.type_original_name)
        else:
            if str(typedef_info.type_kind) == "TypeKind.RECORD":
                typedef_info.type_kind = None
            else:
                typedef_info.type_kind = blend2d_parser.get_cindex_ctypes_mapping(str(typedef_info.type_kind), typedef_info.original_name)
...
def sanitize_function(ctx):
    for func_name, func_info in ctx.decl_functions.items():
        ...
        for arg in func_info.args:
            if blend2d_parser.is_blend2d_callback_type(arg.type_kind, arg.type_name):
                arg.type_kind = ":" + arg.type_name
            else:
                arg.type_kind = blend2d_parser.get_cindex_ctypes_mapping(str(arg.type_kind), arg.type_name)
        func_info.retval.type_kind = blend2d_parser.get_cindex_ctypes_mapping(str(func_info.retval.type_kind), func_info.retval.type_original_name)
...
def generate_function(ctx, indent = "", setup_method_name = ""):
    ...
    for func_name, func_info in ctx.decl_functions.items():
        ...
        if len(func_info.args) > 0:
            ...
            arg_is_record = lambda arg: blend2d_parser.query_blend2d_cindex_mapping_entry_exists(arg) and blend2d_parser.get_blend2d_cindex_mapping_value(arg) == "TypeKind.RECORD"
            args_ctype_list = list(map((lambda arg: re.sub('^BL_', '', arg) + ".by_value" if arg_is_record(arg) else arg), args_ctype_list))
