...
def generate_type_mapping(headers_list_filename = './blend2d_headers_list.json', headers_dir = '../blend2d_dll/blend2d/src/blend2d/'):
...
def generate_define_list(headers_list_filename = './blend2d_headers_list.json', headers_dir = '../blend2d_dll/blend2d/src/blend2d/', concatinate = True):
    headers_list_file = Path(headers_list_filename)
...
def _init_define_mapping(mapping_filename = './blend2d_define_mapping.json'):
...
def _init_type_mapping(mapping_filename = './blend2d_cindex_mapping.json'):
    mapping_file = Path(mapping_filename)
...
def register_blend2d_cindex_mapping(strTypeKind, strBlend2dTypedef):
    if cindex_mapping == None:
        _init_type_mapping()
    if strBlend2dTypedef not in cindex_mapping:
        cindex_mapping[strBlend2dTypedef] = strTypeKind

def query_blend2d_cindex_mapping_entry_exists(strBlend2dTypedef):
    if cindex_mapping == None:
        _init_type_mapping()
    return strBlend2dTypedef in cindex_mapping

def get_blend2d_cindex_mapping_value(strBlend2dTypedef):
    if cindex_mapping == None:
        _init_type_mapping()
    return cindex_mapping[strBlend2dTypedef]


def get_cindex_ctypes_mapping(strTypeKind, strBlend2dTypedef):

    if strTypeKind == 'TypeKind.RECORD':
        return strBlend2dTypedef

    isPointerToChar = False
    if strTypeKind == 'TypeKind.POINTER':
        pattern = re.compile(r"\s*char\s*\*")
        m = re.search(pattern, strBlend2dTypedef)
        if m:
    ...
    if isPointerToChar:
        ...
        if strTypeKind != 'TypeKind.TYPEDEF':
            ...
            mapping_key = get_blend2d_cindex_mapping_value(strBlend2dTypedef)

        if mapping_key == "TypeKind.RECORD":
            return str(strBlend2dTypedef)
        else:
            return ctypes_mapping[mapping_key]

def is_blend2d_callback_type(strTypeKind, strBlend2dTypedef):
    return query_blend2d_cindex_mapping_entry_exists(strBlend2dTypedef) and (get_blend2d_cindex_mapping_value(strBlend2dTypedef) == "TypeKind.FUNCTIONPROTO")
...
class ParseContext(object):
    ...
    def __init__(self, filename = "", headers_dir = '../blend2d_dll/blend2d/src/blend2d/'):
        ...
        self.decl_typedefs = {}
        self.decl_functions = {}
        self.parse_file = headers_dir + filename
...
def collect_decl_typedef(ctx, cursor):
    ...
    match_obj = re.match(r"^BL_(.+)", typedef_info.original_name)
    if match_obj:
        # Remove prefix 'BL_' from name
    ...
    if underlying_type.kind in {TypeKind.CONSTANTARRAY, TypeKind.INCOMPLETEARRAY, TypeKind.VARIABLEARRAY, TypeKind.DEPENDENTSIZEDARRAY}:
        ...
        if canonical_type.kind == TypeKind.FUNCTIONPROTO:
            ...
            match_obj = re.match(r"^BL_(.+)", retval_info.type_original_name)
            if match_obj:
                # Remove prefix 'BL_' from name
...
def collect_decl_struct(ctx, cursor, struct_name=None, typedef_name=None):
    ...
    for field in cursor.get_children():
        ...
        match_obj = re.match(r"^BL_(.+)", field_info.type_original_name)
        if match_obj:
            # Remove prefix 'BL_' from struct name and capitalize first character
        ...
        if canonical_kind in {TypeKind.CONSTANTARRAY, TypeKind.INCOMPLETEARRAY, TypeKind.VARIABLEARRAY, TypeKind.DEPENDENTSIZEDARRAY}:
            # ex.) char text[BL_TEXTEDITINGEVENT_TEXT_SIZE];
            # ex.) Uint8 padding[56];
    ...
    match_obj = re.match(r"^BL_(.+)", struct_info.original_name)
    if match_obj:
...
def collect_decl_function(ctx, cursor):
    ...
    match_obj = re.match(r"^BL_(.+)", func_info.original_name)
    if match_obj:
    ...
    for arg in args:
        ...
        match_obj = re.match(r"^BL_(.+)", arg_info.original_name)
        if match_obj:
            # Remove prefix 'SDL_' from name
...
def execute(ctx):
    ...
    try:
        ...
        for typedef_name, typedef_info in ctx.decl_typedefs.items():
            register_blend2d_cindex_mapping(str(typedef_info.type_kind), typedef_info.api_name)
